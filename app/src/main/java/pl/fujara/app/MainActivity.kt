...
private fun HomeScreen(
    ...
    ScreenContainer {
        ...
        OutlinedButton(
            ...
            Text(
                if (showDemo) {
                    tx(language, "Ukryj demo", "Hide demo", "Сховати демо", "Скрыть демо")
                } else {
                    tx(language, "Pokaż demo obliczeń", "Show calculation demo", "Показати демо розрахунків", "Показать демо расчётов")
                }
            )
        }

        if (showDemo) {
            DemoAnalysisCard(language)
        }
        ...
        Column(
            ...
            Row(
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text(
                        text = tx(language, "Przykładowa oferta", "Sample offer", "Приклад пропозиції", "Пример предложения"),
                        color = Color(0xFFB9BEC8),
                        style = MaterialTheme.typography.bodySmall
                    )
                }
                Text(
                    text = tx(language, "OPŁACA SIĘ", "WORTH IT", "ВИГІДНО", "ВЫГОДНО"),
                    color = Color(0xFF53CD73),
                    fontWeight = FontWeight.Bold
                )
            }

            HorizontalDivider(color = Color(0xFF343840))
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                ...
                DemoMetric(
                    modifier = Modifier.weight(1f),
                    label = tx(language, "PO KOSZTACH", "AFTER COSTS", "ПІСЛЯ ВИТРАТ", "ПОСЛЕ РАСХОДОВ"),
                    value = "19,46 zł",
                    highlight = true
                )
            }
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                DemoMetric(
                    modifier = Modifier.weight(1f),
                    label = tx(language, "TRASA", "ROUTE", "МАРШРУТ", "МАРШРУТ"),
                    value = "6,8 km"
                )
                DemoMetric(
                    modifier = Modifier.weight(1f),
            ...
            Row(modifier = Modifier.fillMaxWidth()) {
                DemoMetric(
                    modifier = Modifier.weight(1f),
                    label = tx(language, "PO KOSZT./H", "AFTER COSTS/H", "ПІСЛЯ ВИТР./Г", "ПОСЛЕ РАСХ./Ч"),
                    value = "41,70 zł/h",
                    highlight = true
                )
                DemoMetric(
                    modifier = Modifier.weight(1f),
                    label = tx(language, "PO KOSZT./KM", "AFTER COSTS/KM", "ПІСЛЯ ВИТР./КМ", "ПОСЛЕ РАСХ./КМ"),
                    value = "2,86 zł/km",
                    highlight = true
                )
            }
            ...
            Text(
                text = tx(
                    language,
                    "To tylko demo dla Ciebie i recenzenta Google Play — nie pochodzi z prawdziwego zlecenia.",
                    "This is only a demo for you and the Google Play reviewer — it does not come from a real offer.",
                    "Це лише демо для вас і рецензента Google Play — воно не походить із реальної пропозиції.",
                    "Это только демо для вас и рецензента Google Play — оно не получено из реального предложения."
                ),
                color = Color(0xFFB9BEC8),
                style = MaterialTheme.typography.bodySmall
            )
...
private fun DemoMetric(
    ...
    Column(modifier = modifier) {
        ...
        Text(
            text = value,
            color = if (highlight) Color(0xFF53CD73) else Color.White,
            fontWeight = FontWeight.Bold
        )
    }
}
